...
Eve connects to the `relay` orchestrator's frontend socket for all backend operations; relay proxies onward to relayLLM/relayScheduler. In orchestrator-managed deployments relay injects `RELAY_FRONTEND_SOCKET`. For standalone TCP/dev use, set the URL via environment variable:

```bash
RELAY_FRONTEND_URL=http://localhost:3001 npm start
```
...
```
Browser ──WS──► Eve ──WS──► relay ──► relayLLM       (sessions, messages, permissions)
...
Eve sits between a single browser user and the `relay` orchestrator, which fronts a set of trusted backend services (`relayLLM`, `relayScheduler`, on-device TTS/STT daemons). Two trust boundaries need to be hardened: browser↔Eve and Eve↔backend.
...
Eve's only outbound channel is a single connection to the `relay` orchestrator's **frontend socket**. relay authenticates each request and reverse-proxies it to whichever managed service registered the route — `relayLLM` for sessions/models/permissions, `relayScheduler` for tasks — injecting that service's own internal token; relay serves project/MCP routes itself. Eve never dials relayLLM or relayScheduler directly. The transport is a **Unix domain socket + ephemeral bearer token**, reusing the pattern the Go `relay` orchestrator uses for MCP tool calls (`relay/service_registry.go`, `relay/frontend_server.go`, `relay/bridge/server.go`).

- **Socket mode (preferred).** `RELAY_FRONTEND_SOCKET` points at a `0600`-permission Unix socket allocated by the relay orchestrator at spawn time. Kernel-enforced file permissions do the primary authorization work; the bearer token is defense-in-depth.
- **TCP mode (fallback).** `RELAY_FRONTEND_URL` is used only when Eve and relay run on different hosts. Must be `https://`, with certificate verification enabled (`rejectUnauthorized: true`) and an optional `RELAY_FRONTEND_CA` for operators who run an internal CA. Plain `http://` off loopback is refused at startup — no silent downgrade.
- **Bearer token.** `RELAY_FRONTEND_TOKEN` is a 32-byte hex value generated by the relay orchestrator at spawn time; relay's frontend listener holds one end and the value is injected into Eve via environment variable. It is ephemeral — revoked automatically when Eve exits, never written to disk. Eve sends it on every outbound HTTP request as `Authorization: Bearer <token>` and during the WebSocket upgrade as the same header, so relay rejects unauthenticated upgrades before protocol-switching. relay then injects a separate internal token to reach each backend service.
- **Parallel, not multiplexed.** The MCP bridge socket (`RELAY_MCP_TOKEN`) carries tool-call JSON; the Eve↔relay frontend runs on its own socket with its own token, so a leaked token in one channel cannot grant access to the other.
...
> **Implementation status.** Shipped. Design rationale and verification results (including a Chrome end-to-end test over the Unix socket) live in [`plans/cozy-honking-toast.md`](plans/cozy-honking-toast.md). The cross-repo pieces — the frontend bearer middleware + Unix socket listener in `../relay/frontend_server.go`, the reverse-proxy + internal-token injection in `../relay/frontend_dispatcher.go` and `../relay/enhanced_services.go`, and the internal-socket listener in `../relayLLM` — landed alongside the Eve changes.
...
routes/index.js          - HTTP proxy to relay's frontend socket (models, projects, sessions, tasks)
...
| Variable | Default | Description |
|----------|---------|-------------|
| `RELAY_FRONTEND_SOCKET` | - | Path to relay's frontend Unix domain socket, provisioned by the `relay` orchestrator at spawn time. When set, Eve dials it via `http.Agent({ socketPath })` for both HTTP and WebSocket traffic. Kernel file permissions (`0600`) anchor authorization; no TLS is needed on this hop. Preferred transport in orchestrator-managed deployments. |
| `RELAY_FRONTEND_URL` | `http://localhost:3001` | Fallback TCP URL for split-host deployments. Off-loopback values must use `https://`; plain `http://` to a remote host is refused at startup. |
| `RELAY_FRONTEND_TOKEN` | - | Ephemeral bearer token for the Eve↔relay frontend channel, injected by the orchestrator into Eve at spawn time. Sent on every HTTP request and every WebSocket upgrade as `Authorization: Bearer <token>`. Per-process-lifetime — never written to disk. |
| `RELAY_FRONTEND_CA` | - | Optional PEM path for operators running an internal CA in front of relay's frontend. Loaded into a shared `https.Agent` with `rejectUnauthorized: true`. There is no "skip verify" option. |
...
- **[Relay](https://github.com/barelyworkingcode/relay)** -- MCP orchestrator. Manages Eve as a background service with per-token permissions, and fronts all of Eve's backend traffic: Eve connects to relay's frontend socket, which reverse-proxies to relayLLM and relayScheduler (and serves project/MCP routes itself).
- **[relayLLM](https://github.com/barelyworkingcode/relayLLM)** -- LLM engine. Handles all session, model, and permission operations. Reached through relay's frontend, not directly.
- **[relayScheduler](https://github.com/barelyworkingcode/relayScheduler)** -- Task scheduler. Runs LLM prompts on schedule. Eve reaches it through relay's frontend, which dispatches `/api/tasks` to it directly.
