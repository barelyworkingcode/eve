...
- **`model`** (optional) — preferred model for AI calls. Use `claude-haiku-4-5` for cheap/fast structured-data tasks (parsing, classification, format conversion). Use `claude-sonnet-5` or `claude-opus-5-5` only when the module genuinely needs reasoning.
