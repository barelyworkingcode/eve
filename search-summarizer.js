...
function buildPrompt(query, matches, projectName) {
  ...
  const lines = [
    ...
    '',
    'Write a brief prose summary that helps them decide where to click: which modules or areas the matches cluster in, any obvious pattern (definitions, call sites, tests, docs), and anything that looks like an outlier worth a look. Name areas rather than walking through every file.',
    '',
    'The summary is displayed as plain text, so markdown, headings, and bullets would show up as raw symbols.',
    '',
